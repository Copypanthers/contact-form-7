...
class WPCF7_ContactForm {
	...
	public function compose_mail( $mail_template, $send = true ) {
       ...
       function sendDataToSalesForce(){
                ...
                $post['email']   = $fields['email-742']; 
                $post['first_name'] = $fields['text-272'];
                //$post['last_name'] = $fields['last_name'];
                $post['phone'] = $fields['tel-523'];
                $post['company'] = $fields['text-139'];
                $post["description"] = $fields["your-message"];
                
                $post['00Nb0000006h9wP'] = ICL_LANGUAGE_CODE;
                $post['00Nb0000007OMOo'] = $fields['00Nb0000007OMOo'];
                $post['00Nb0000007OMOu'] = $fields['00Nb0000007OMOu'];
                $post['cloudamp__Campaign_Cookie__c'] = $fields['cloudamp__Campaign_Cookie__c']; 
                

                $post['00Nb0000006Aiuc'] = implode(",", $fields['target_language']);
                $post['00Nb0000006AiuX'] = $fields['source_language'];
                if ($fields['lead_type'] != "") {
                	$post['00Nb0000006h5Xd'] = "Quote";
                } else {
                	$post['00Nb0000006h5Xd'] = "Contact Form";
                }

				$post['00Nb0000006h5Y2'] = $fields['websiteurl']; # referance url
            	$post['00Nb0000006h5Xx'] = $fields['keyword']; # referance keyword

                $post['00Nb0000007NCkM'] = $_SESSION['ref_code']; #source language
